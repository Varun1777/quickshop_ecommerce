...
const HomePage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchHomeData = async () => {
      try {
        ...
        setFeaturedProducts(featuredData.products.filter(product => product.rating >= 4.5).slice(0, 10));

        // Get products with highest discount for flash deals
        const dealsData = await getProducts({
          limit: 20
        });
        setFlashDeals(dealsData.products.sort((a, b) => b.discountPercentage - a.discountPercentage).slice(0, 4));
  ...
  return <div>
      ...
      <div className="container mx-auto px-4 py-12 space-y-16">
        ...
        {/* Featured Products */}
        <section>
          <FeaturedProductsCarousel products={featuredProducts} isLoading={isLoading} title="Featured Products" subtitle="Top picks based on customer ratings and feedback" />
        </section>
        ...
        {/* New Arrivals */}
        <section>
          <FeaturedProductsCarousel products={newArrivals} isLoading={isLoading} title="New Arrivals" subtitle="Check out our latest products" />
        </section>
        ...
        <section className="bg-brand-50 py-12 -mx-4 px-4 rounded-lg">
          <div className="container mx-auto max-w-2xl text-center">
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              <input type="email" placeholder="Enter your email address" className="flex-1 px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-brand-500" />
